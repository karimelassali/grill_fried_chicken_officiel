  ...
  Object.keys(timeLeft).forEach((interval) => {
    ...
    timerComponents.push(
      <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        className="flex backdrop-blur-lg rounded-3xl p-4 md:p-5 flex-col items-center mx-2 md:mx-3 bg-gradient-to-b from-slate-800/90 to-gray-900/90 border-2 border-amber-500/60 shadow-2xl relative overflow-hidden group"
      >
        ...
        <span className="text-xs uppercase text-amber-300 mt-2 font-bold tracking-wider relative z-10 drop-shadow-lg">
          {interval}
        </span>
      </motion.div>
    );
    ...
    <div className="flex justify-center my-4">
      {timerComponents.length ? timerComponents : (
        <motion.div
          ...
          className="text-3xl font-bold text-amber-400 bg-gradient-to-r from-amber-600/20 to-red-600/20 px-8 py-4 rounded-2xl border border-amber-500/30"
        >
          🎉 We're Open! 🎉
        </motion.div>
      )}
